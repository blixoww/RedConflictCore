 ...
 */
public class ShopEventCommand extends CoreCommand {
    ...
    public ShopEventCommand(JavaPlugin plugin, ShopEventManager manager, ShopDatabase database) {
        super(plugin, "shopevent", false);
    ...
    protected void execute(CommandSender sender, String label, String[] args) {
        ...
        switch (sub) {
            ...
            case "reload":
                manager.reload();
                sender.sendMessage(PRE + "Config rechargée.");
                return;
            case "krach":
                cmdKrach(sender, args);
                return;
            case "inflation":
                cmdInflation(sender, args);
                return;
            case "aubaine":
                cmdAubaine(sender, args);
                return;
            case "stop":
                cmdStop(sender, args);
                return;
    ...
    private void cmdNext(CommandSender s) {
    ...
    private void cmdKrach(CommandSender s, String[] args) {
    ...
    private void cmdInflation(CommandSender s, String[] args) {
        int dur = parseDur(args, 1);
    ...
    private void cmdAubaine(CommandSender s, String[] args) {
        if (args.length < 2) {
        ...
        try {
            ...
            if (si == null) {
                s.sendMessage(PRE + ChatColor.RED + "Item id introuvable : " + itemId);
                return;
            }
            ...
            if (found == null) {
                s.sendMessage(PRE + ChatColor.RED + "Item non trouvé : " + args[1]);
                return;
            }
            itemId = found.id;
        }
        ...
        if (args[2].equalsIgnoreCase("up")) upward = true;
            ...
            s.sendMessage(PRE + ChatColor.RED + "up ou down attendu.");
            return;
        }
        ...
        try {
            ...
            s.sendMessage(PRE + ChatColor.RED + "Multiplicateur invalide.");
            return;
        }
        if (mult <= 0 || mult > 10) {
            s.sendMessage(PRE + ChatColor.RED + "Multiplicateur entre 0.01 et 10.");
            return;
        }
    ...
    private void cmdStop(CommandSender s, String[] args) {
        if (args.length < 2) {
            s.sendMessage(PRE + ChatColor.RED + "Usage: /shopevent stop <krach|inflation|aubaine|all>");
            return;
        }
    ...
    public List<String> onTabComplete(CommandSender sender, Command cmd, String alias, String[] args) {
        ...
        if (args.length == 1) {
            return Stream.of("list", "next", "reload", "krach", "inflation", "aubaine", "stop")
                    .filter(s -> s.startsWith(args[0].toLowerCase(Locale.ROOT)))
                    .collect(Collectors.toList());
        }
        if (args.length == 2 && args[0].equalsIgnoreCase("aubaine")) {
            return Stream.of("random")
                    .filter(s -> s.startsWith(args[1].toLowerCase(Locale.ROOT)))
                    .collect(Collectors.toList());
        }
        if (args.length == 2 && args[0].equalsIgnoreCase("stop")) {
            return Stream.of("all", "krach", "inflation", "aubaine")
                    .filter(s -> s.startsWith(args[1].toLowerCase(Locale.ROOT)))
                    .collect(Collectors.toList());
        }
