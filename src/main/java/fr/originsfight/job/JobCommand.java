...
public class JobCommand implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        if (sub.equals("info") && sender.hasPermission("jobs.admin")) {
            if (args.length < 2) { sender.sendMessage("§cUsage: /metier info <joueur>"); return true; }
        ...
        if (sub.equals("xp") && sender.hasPermission("jobs.admin")) {
            if (args.length < 5 || !args[1].equalsIgnoreCase("add")) {
                sender.sendMessage("§cUsage: /metier xp add <joueur> <metier> <montant>"); return true;
            }
            Player target = Bukkit.getPlayer(args[2]);
            if (target == null) { sender.sendMessage("§cJoueur introuvable."); return true; }
            JobType jt = JobType.fromString(args[3]);
            if (!jt.isReal()) { sender.sendMessage("§cMétier inconnu. Valides : MINER, FARMER, ARTISAN"); return true; }
            int amount;
            try { amount = Integer.parseInt(args[4]); } catch (NumberFormatException e) {
                sender.sendMessage("§cMontant invalide."); return true;
            }
        ...
        if (sub.equals("reset") && sender.hasPermission("jobs.admin")) {
            if (args.length < 3) { sender.sendMessage("§cUsage: /metier reset <metier> <joueur>"); return true; }
            JobType jt = JobType.fromString(args[1]);
            if (!jt.isReal()) { sender.sendMessage("§cMétier inconnu."); return true; }
            Player target = Bukkit.getPlayer(args[2]);
            if (target == null) { sender.sendMessage("§cJoueur introuvable."); return true; }
